...
FillType ThreadedContourGenerator::default_fill_type()
{
...
LineType ThreadedContourGenerator::default_line_type()
{
...
index_t ThreadedContourGenerator::find_look_S(index_t look_N_quad) const
{
...
py::tuple ThreadedContourGenerator::get_chunk_count() const
{
...
py::tuple ThreadedContourGenerator::get_chunk_size() const
{
...
const double& ThreadedContourGenerator::get_point_x(index_t point) const
{
    assert(point >= 0 && point < _n && "point index out of bounds");
    return _x.data()[point];
}

const double& ThreadedContourGenerator::get_point_y(index_t point) const
{
    assert(point >= 0 && point < _n && "point index out of bounds");
    return _y.data()[point];
}

const double& ThreadedContourGenerator::get_point_z(index_t point) const
{
    assert(point >= 0 && point < _n && "point index out of bounds");
    return _z.data()[point];
}

void ThreadedContourGenerator::init_cache_grid(const MaskArray& mask)
{
    index_t i, j, quad;
    if (mask.ndim() == 0) {
...
void ThreadedContourGenerator::interp(
...
py::sequence ThreadedContourGenerator::lines(const double& level)
{
    _filled = false;
...
void ThreadedContourGenerator::move_to_next_boundary_edge(
...
void ThreadedContourGenerator::set_look_flags(index_t hole_start_quad)
{
    assert(_identify_holes);
...
bool ThreadedContourGenerator::supports_fill_type(FillType fill_type)
{
    switch (fill_type) {
        case FillType::OuterCodes:
        case FillType::OuterOffsets:
...
bool ThreadedContourGenerator::supports_line_type(LineType line_type)
{
    switch (line_type) {
        case LineType::Separate:
        case LineType::SeparateCodes:
...
void ThreadedContourGenerator::write_cache() const
{
...
void ThreadedContourGenerator::write_cache_quad(index_t quad) const
{
