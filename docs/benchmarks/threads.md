...
particularly well.  The speedup with 6 threads is 2.4-2.5 for both {py:meth}`~.ContourGenerator.lines`
and {py:meth}`~.ContourGenerator.filled`.  This problem dataset is perhaps not
...
Using 6 threads the speedup is 4.2-4.5 for {py:meth}`~.ContourGenerator.lines` and 4.6-5.2 for
{py:meth}`~.ContourGenerator.filled`.

The {py:class}`~.LineType` and {py:class}`~.FillType` options that do not scale well are those that return individual
...
```{note}
   ...
   If they are only needed for rendering using the {{ Matplotlib }} Agg renderer, then for complicated
