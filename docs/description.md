...
change during each call to {py:meth}`~.ContourGenerator.lines` and
{py:meth}`~.ContourGenerator.filled`.
...
At the start of each call to {py:meth}`~.ContourGenerator.lines` or
{py:meth}`~.ContourGenerator.filled`, the non-grid cache flags are set. These are the
...
1. Counts the total number of points and lines followed.
2. Erases start flags that are no longer needed.
3. If it is calculating filled contours and the {py:class}`~.FillType` requires the relationship
   between outer boundary and their holes, flags are set in the cache for this.
4. Sets extra cache flags for rows in the domain that do not contain any start flags to speed up
   the second pass.

At the end of the first pass a buffer is allocated to contain all of the contour points, and one or
more offset buffers depending upon the {py:class}`~.LineType` or {py:class}`~.FillType`.

The second pass does the following:

1. Calculates and stores the contour points.
2. Stores the offsets of the starts and ends of each contour line.
3. Stores the relationship between the outer boundaries and their holes, if required.
