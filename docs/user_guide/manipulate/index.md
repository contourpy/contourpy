...
The format of contour data returned by {py:meth}`~.ContourGenerator.lines`,
{py:meth}`~.ContourGenerator.filled`, {py:meth}`~.ContourGenerator.multi_lines` and
{py:meth}`~.ContourGenerator.multi_filled` is controlled by the `line_type`, `fill_type` and
chunking keyword arguments passed to {py:func}`~.contour_generator`. Usually this is fully
