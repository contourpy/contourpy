...
{py:func}`~.convert_lines` and {py:func}`~.convert_multi_lines` are used to convert contour lines to
a different {py:class}`~.LineType`.
...
Any {py:class}`~.LineType` can be converted to any other {py:class}`~.LineType`.
...
{py:func}`~.convert_filled` and {py:func}`~.convert_multi_filled` are used to convert filled
contours to a different {py:class}`~.FillType`.
...
Not all {py:class}`~.FillType` can be converted to any other {py:class}`~.FillType`.
Two of the {py:class}`~.FillType` (`FillType.ChunkCombinedCode` and
`FillType.ChunkCombinedOffset`) do not include information about the relationship between outer
and inner boundaries so they cannot be converted to any of the {py:class}`~.FillType` that
