...
{py:class}`~.LineType` is an enum that is passed as the `line_type` keyword argument to
{py:func}`~.contour_generator()` and is used to specify the format of data returned from calls
to {py:meth}`~.ContourGenerator.lines`. If not explicitly specified then the default is
...
The format of data returned by {py:meth}`~.ContourGenerator.lines` for each of the
possible {py:class}`~.LineType` members is best illustrated through an example.
...
This is the line type used internally by {{ Bokeh }} and {{ HoloViews }}.

## How to choose which line type to use

1. Do you need {{ Matplotlib }} kind codes?

2. Do you want each line's points in a separate array or combined together?
...
```{note}
   The order of lines returned by a particular {py:meth}`~.ContourGenerator.lines` call
