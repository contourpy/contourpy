...
argument to {py:func}`~.contour_generator`. It can be specified in any form that is
...
````{warning}
   If the `z` array does not need to be copied then both the {py:class}`~.ContourGenerator`
   and your client code have access to the same shared array. This means that you can purposefully
   or accidentally alter the data that the {py:class}`~.ContourGenerator` is using, which is
   ...
   that the {py:class}`~.ContourGenerator` is using this `z` array.
   ...
   Now change an element of the `z` array that is used by the {py:class}`~.ContourGenerator`
   and repeat the same {py:meth}`~.ContourGenerator.lines` call:
...
The `z` array passed to {py:func}`~.contour_generator` can be a
[NumPy masked array](https://numpy.org/doc/stable/reference/maskedarray.html) to mask out specific
...
```{note}
   The mask of a `z` array is used only when constructing a {py:class}`~.ContourGenerator`
   object, so there is no danger that a mask shared with client code can subsequently be altered to
   change the behaviour of the {py:class}`~.ContourGenerator`.
```

## Corner mask

The boolean `corner_mask` keyword argument passed to {py:func}`~.contour_generator` is used
