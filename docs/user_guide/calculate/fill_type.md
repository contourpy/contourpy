...
{py:class}`~.FillType` is an enum that is passed as the `fill_type` keyword argument to
{py:func}`~.contour_generator()` and is used to specify the format of data returned from calls
to {py:meth}`~.ContourGenerator.filled`. If not explicitly specified then the default is
...
```{note}
   ...
   {py:meth}`~.ContourGenerator.filled` for all {py:class}`~.FillType` members
...
The format of data returned by {py:meth}`~.ContourGenerator.filled` for each of the
possible {py:class}`~.FillType` members is best illustrated through an example.  This is the
...
```{note}
   The order of boundaries returned by a particular {py:meth}`~.ContourGenerator.filled`
